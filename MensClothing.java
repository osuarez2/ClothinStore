...
public class MensClothing {
    ...
    private void initialize() {
        ...
        myCartButton.addActionListener(e -> {
            if (GlobalSession.getUserId() <= 0) {
                ...
                // Create the base cart content
                JPanel cartContent = ShoppingCart.createCartPage(cardLayout, centerPanel);
                cartContent.setName("CartPage");

                // Create a wrapper panel with BorderLayout to hold cart content + buttons
                ...
                checkoutButton.addActionListener(ev -> {
                    if (GlobalSession.getUserId() <= 0) {
                        notSignedInPopup("You need to sign in to proceed to checkout.");
                    } else {
                        // Navigate to the Checkout page
                        new Checkout();  // Open the Checkout page as a new window
                        frame.dispose(); // Close the current MensClothing window
                    }
                });
    ...
    private JPanel createProductsPage() {
        ...
        JLabel logoLabel = new JLabel("Men's Clothing", SwingConstants.CENTER);
        ...
        try {
            String query = "SELECT item_id, item_name, item_price, item_size, in_stock FROM inventory WHERE dept_id = 1";
            ...
            while (rs.next()) {
                ...
                String priceString = rs.getString("item_price"); // Get price as a string
                if (priceString != null && priceString.startsWith("$")) {
                    priceString = priceString.substring(1); // Remove the dollar sign
                }
    ...
    private void kidsButton(JPanel leftPanel) {
        JButton btn = new JButton("Kids' Clothing");
